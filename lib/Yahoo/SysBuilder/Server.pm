package Yahoo::SysBuilder::Server;
...
sub new {
    ...
    my $self  = {
        verbose     => 0,
        steps_dir   => '/home/y/libexec/ysysbuilder/steps',
        tmp_dir     => '/home/y/tmp/ysysbuilder',
        support_dir => '/home/y/libexec/ysysbuilder/support',
        @_
    };
...
sub _set_path {
    ...
    for my $dir (qw{/usr/local/bin /home/y/bin /sbin /usr/sbin /bin /usr/bin})
    {
        push @path, $dir
...
sub create_ramdisk {
    ...
    $self->_system("gunzip <$spt/ysysbuilder.img >$img")
        or die("$spt/ysysbuilder.img: $!");
...
sub _copy_modules {
    ...
    my $pkg_name = 'ysysbuilder';

    # use yinst for now - think about how this should be done
    # if yinst was not installed
    my @files
        = `yinst list -files $pkg_name | awk '/site_perl/ { print \$1 }'`;
    return if $?;
    return unless @files;
...
use Yahoo::SysBuilder::Utils qw(optional_shell time_repr set_status printbold);
