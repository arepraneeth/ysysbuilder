...
package SysBuilder::ValidateHW;

use strict;
use warnings 'all';
use SysBuilder::Utils qw(run_local fatal_error printbold);
...
sub count_cpus {
    ...
    if ($should_have != $have) {
        if ($should_have > $have) {
            ...
            fatal_error("ERROR: The current profile expects $should_have " .
              "CPUs but $have were detected.");
        } else {
            printbold("WARNING: The current profile expects $should_have " .
              "CPUs but $have were detected.\n");
            return 1; # this is OK
        }
    }
