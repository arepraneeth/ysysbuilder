...
package SysBuilder::SystemFiles;
...
sub configure_ssh_keys {
    ...
    # the build so we will generate new keys now.
    if ( $err ) {
      my $key_gen_err 
        = run_local ( <<'EOT' );
...
sub configure_persistent_net {
    ...
    write_file( "/mnt/etc/udev/rules.d/65-sysbuilder-eth.rules", $rules )
...
sub configure_default_path {
    ...
    if( $cfg->{default_path} ) {
        ...
        $path = "/usr/kerberos/bin:/usr/local/sbin:/usr/local/bin:"
...
EOT
    ...
    write_file( "/mnt/etc/profile.d/sysbuilder.sh", join( "\n", @script ) );
    chmod oct('0755'), '/mnt/etc/profile.d/sysbuilder.sh';
}
...
sub configure_securetty {
    ...
    my @serial_ports = SysBuilder::SerialPort->new->serial_ports;
...
sub configure_passwd {
    ...
    my $root = '### add your own passwd line here ###';
    system(qq(perl -pi -e 's/^root.*/q($root)/e' /mnt/etc/shadow)) == 0
      or die "Failed to set root password"; # not run_local so it doesn't echo
}
...
sub configure_inittab {
    ...
    my $boot_console  = SysBuilder::Console->new->boot_console($cfg);
    my @serial_ports  = SysBuilder::SerialPort->new->serial_ports;
...
sub generate_esx_conf {
    ...
    run_local(
        "cp /sysbuilder/lib/SysBuilder/generate-esx-conf.pl /mnt$dest");
